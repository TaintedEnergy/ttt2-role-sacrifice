...
if SERVER then
  ...
  resource.AddFile("materials/gui/ttt/icon_defi_sacrifice.vmt")
end
...
if CLIENT then
  ...
  SWEP.EquipMenuData = {
    type = "item_weapon",
    name = "Sacrifice's Defib",
    desc = "sacrifice_defi_desc"
  }

  SWEP.Icon = "vgui/ttt/icon_defi_sacrifice"
end
...
if SERVER then
  util.AddNetworkString("RequestSacrificeRevivalStatus")
  util.AddNetworkString("ReceiveSacrificeRevivalStatus")

  function SWEP:SafeRemove()
    hook.Remove("PlayerSwitchWeapon", "TTT2SacrificedefiSwitch")
    self:Remove()
  end
  ...
  function SWEP:Error(type)
    ...
    local error_time = GetConVar("ttt2_sacrificedefi_error_time"):GetFloat()
  ...
  function SWEP:Message(type)
    ...
    if type == DEFI_ERROR_NO_SPACE then
      LANG.Msg(owner, "sacrificedefi_error_no_space", nil, MSG_MSTACK_WARN)
    elseif type == DEFI_ERROR_TOO_FAST then
      LANG.Msg(owner, "sacrificedefi_error_too_fast", nil, MSG_MSTACK_WARN)
    elseif type == DEFI_ERROR_LOST_TARGET then
      LANG.Msg(owner, "sacrificedefi_error_lost_target", nil, MSG_MSTACK_WARN)
    elseif type == DEFI_ERROR_NO_VALID_PLY then
      LANG.Msg(owner, "sacrificedefi_error_no_valid_ply", nil, MSG_MSTACK_WARN)
    elseif type == DEFI_ERROR_ALREADY_REVIVING then
      LANG.Msg(owner, "sacrificedefi_error_already_reviving", nil, MSG_MSTACK_WARN)
    elseif type == DEFI_ERROR_FAILED then
      LANG.Msg(owner, "sacrificedefi_error_failed", nil, MSG_MSTACK_WARN)
    elseif type == DEFI_ERROR_THRALL then
      LANG.Msg(owner, "sacrificedefi_error_thrall", nil, MSG_MSTACK_WARN)
    elseif type == DEFI_ERROR_PLAYER_ALIVE then
      LANG.Msg(owner, "sacrificedefi_error_player_alive", nil, MSG_MSTACK_WARN)
    end
  end
  ...
  function SWEP:BeginRevival(ragdoll, bone)
    ...
    local reviveTime = GetConVar("ttt2_sacrificedefi_revive_time"):GetFloat()
    -- local reviveTime = 3.0

    self:SetState(DEFI_BUSY)
    self:SetStartTime(CurTime())
    self:SetReviveTime(reviveTime)
    hook.Add("PlayerSwitchWeapon", "TTT2SacrificedefiSwitch", function(ply, old, new)
      if old:GetClass() == "weapon_ttt_sacrificedefi" then
        return true 
      else
        hook.Remove("PlayerSwitchWeapon", "TTT2SacrificedefiSwitch")
      end
    ...
    ply:Revive(
      ...
      function()
        ...
        ply:SetHealth(GetConVar("ttt2_sacrifice_revive_target_hp"):GetInt())
        --events.Trigger(EVENT_SACRIFICE_DEFIB, owner, ply)
        SendFullStateUpdate()
      end,
      function(p)
        ...
        elseif not IsValid(p) or (p:Alive() and not (SpecDM and p:IsGhost())) then
          ...
          self:FinishRevival()
          owner:TakeDamage(GetConVar("ttt2_sacrifice_revive_damage"):GetInt(), game.GetWorld())
          return true
        end
      end,
      true,
    ...
    ply:SendRevivalReason("revived_by_sacrifice", {name = self:GetOwner():Nick()})
  ...
  function SWEP:Think()
    ...
    local owner = self:GetOwner()
    local revive_time = GetConVar("ttt2_sacrificedefi_revive_time"):GetFloat()
  ...
  function SWEP:Initialize()
    local ammo = GetConVar("ttt2_sacrificedefi_ammo"):GetInt()
    self:SetClip1(ammo)
  end
  ...
  function SWEP:PrimaryAttack()
    ...
    if CORPSE.GetPlayer(ent):GetSubRole() == ROLE_THRALL and not GetConVar("ttt2_sacrificedefi_res_thrall"):GetBool() then
  ...
  net.Receive("RequestSacrificeRevivalStatus", function(_, requester)
    local ply = net.ReadEntity()

    if not IsValid(ply) then return end

    net.Start("ReceiveSacrificeRevivalStatus")
    net.WriteEntity(ply)
...
if CLIENT then
  ...
  local function IsPlayerReviving(ply)
    if not ply.defi_lastRequest or ply.defi_lastRequest < CurTime() + 0.3 then
      net.Start("RequestSacrificeRevivalStatus")
      net.WriteEntity(ply)
      net.SendToServer()
  ...
  net.Receive("ReceiveSacrificeRevivalStatus", function()
    local ply = net.ReadEntity()
  ...
  hook.Add("TTTRenderEntityInfo", "ttt2_sacrifice_defibrillator_display_info", function(tData)
    ...
    if not IsValid(activeWeapon) or activeWeapon:GetClass() ~= "weapon_ttt_sacrificedefi" then return end
    ...
    if activeWeapon:GetState() == DEFI_CHARGE then
      tData:AddDescriptionLine(
        LANG.TryTranslation("sacrificedefi_charging"),
        COLOR_ORANGE
      )
    ...
    if activeWeapon:GetState() ~= DEFI_BUSY and IsValid(ply) and IsPlayerReviving(ply) then
      tData:AddDescriptionLine(
        LANG.TryTranslation("sacrificedefi_player_already_reviving"),
        COLOR_ORANGE
      )
    ...
    tData:AddDescriptionLine(
      LANG.GetParamTranslation("sacrificedefi_hold_key_to_revive", {key = Key("+attack", "LEFT MOUSE")}),
      colorGreen
    )
    ...
    tData:AddDescriptionLine(
      LANG.GetParamTranslation("sacrificedefi_revive_progress", {time = math.Round(timeLeft, 1)}),
      colorGreen
    )
